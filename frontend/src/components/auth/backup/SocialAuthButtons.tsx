...
export default function SocialAuthButtons({ 
  ...
  const handleSocialAuth = async (provider: 'google' | 'facebook' | 'apple') => {
    try {
      ...
      // Check for popup blockers before attempting authentication
      if (isPopupBlocked()) {
        showHelp() // Show help dialog proactively
        toast('Popup blocker detected. Please click "Allow" in your browser and try again', {
          icon: '🔒',
          duration: 5000
        })
        return // Don't proceed with auth if popups are blocked
      }
      ...
      if (errorMessage.includes('popup')) {
        ...
        // Show help dialog when popup is blocked
